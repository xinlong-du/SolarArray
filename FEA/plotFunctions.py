...
def springResPlot(springRes,springID):
    fig = plt.figure(figsize=big_fig_size)
    ...
    ax4.tick_params(direction="in")
    ax5.tick_params(direction="in")
    ax0.plot(springRes[:,0],springRes[:,6],linewidth=plt_line_width)
    ax1.plot(springRes[:,1],springRes[:,7],linewidth=plt_line_width)
    ax2.plot(springRes[:,2],springRes[:,8],linewidth=plt_line_width)
    ax3.plot(springRes[:,3],springRes[:,9],linewidth=plt_line_width)
    ax4.plot(springRes[:,4],springRes[:,10],linewidth=plt_line_width)
    ax5.plot(springRes[:,5],springRes[:,11],linewidth=plt_line_width)
    ax0.set_xlabel('X Disp. (m)',fontsize=fig_font_size)
    ax0.set_ylabel('Force (N)',fontsize=fig_font_size)
    ax1.set_xlabel('Y Disp. (m)',fontsize=fig_font_size)
    ax1.set_ylabel('Force (N)',fontsize=fig_font_size)
    ax2.set_xlabel('Z Disp. (m)',fontsize=fig_font_size)
    ax2.set_ylabel('Force (N)',fontsize=fig_font_size)
    ax3.set_xlabel('X Rotation (rad)',fontsize=fig_font_size)
    ax3.set_ylabel('Moment (N*m)',fontsize=fig_font_size)
    ax4.set_xlabel('Y Rotation (rad)',fontsize=fig_font_size)
    ax4.set_ylabel('Moment (N*m)',fontsize=fig_font_size)
    ax5.set_xlabel('Z Rotation (rad)',fontsize=fig_font_size)
    ax5.set_ylabel('Moment (N*m)',fontsize=fig_font_size)
    plt.title('Spring '+springID+' response')
    plt.savefig('./Data/springOutput/springResponse'+springID+'.tif', transparent=False, bbox_inches='tight', dpi=100)