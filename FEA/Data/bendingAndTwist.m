...
for i=0:30:330
    for j=9:9
        filename=strcat('./testAllcases2/dir',num2str(i),'spd',num2str(j),'nodeDisp.out');
        nodeDisp=load(filename);
        filename=strcat('./testAllcases2/dir',num2str(i),'spd',num2str(j),'eleForce.out');
        eleForce=load(filename);
        filename=strcat('./testAllcases2/dir',num2str(i),'spd',num2str(j),'springResp.out');
...
maxDisp=zeros(8,1);
for i=1:8
    maxDisp(i)=39.37*max(ndLocYsAll{i});
end

minDisp=zeros(8,1);
for i=1:8
    minDisp(i)=39.37*min(ndLocYsAll{i});
end
...
set(hfig,'PaperUnits','inches');
set(hfig,'PaperPosition',[0 0 figWidth figHeight]);
fileout='.\testAllCases2\rot1621-2.';
print(hfig,[fileout,'tif'],'-r200','-dtiff');
